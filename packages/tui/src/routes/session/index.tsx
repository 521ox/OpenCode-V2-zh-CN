    ...
    },
    {
      ...
      run: async () => {
        try {
          ...
          const content = formatSessionTranscript(transcript.info, transcript.messages, true, t)
        ...
        try {
          ...
          const content =
            options.format === "markdown"
              ? formatSessionTranscript(transcript.info, transcript.messages, options.thinking, t, options.tools)
    ...
    <box ref={setTarget} id={sessionRowID(props.row, props.boundaryID)} marginTop={1} flexShrink={0}>
      <Switch>
        <Match when={props.row.type === "group" ? props.row : undefined}>
          {(row) => (
            <SessionGroupView
              pendingTools={(data.session.permission.list(ctx.sessionID) ?? []).flatMap((request) =>
          ...
          <text fg={hover() ? theme.text.base : theme.text.muted} wrapMode="none">
            ...
            <span>
              {t(summary().count === 1 ? "session.tokenSummaryOne" : "session.tokenSummary", {
                count: summary().count,
                fresh: summary().latestNewTokens.toLocaleString(),
                cached: summary().latestCached.toLocaleString(),
                total: summary().latestTotal.toLocaleString(),
              })}
            </span>
