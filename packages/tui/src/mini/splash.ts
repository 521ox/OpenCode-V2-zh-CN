...
function title(text: string | undefined, mono = false, locale?: string): string {
  if (!text) {
    return translate(resolveLocale(locale), "miniCli.untitled")
  }
...
function buildExit(input: SplashWriterInput, ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  const session = translate(resolveLocale(input.locale), "miniCli.session")
  const label = translate(resolveLocale(input.locale), "miniCli.continue")
...
export function splashMeta(input: SplashInput): SplashMeta {
  return {
    title: title(input.title, input.mono, input.locale),
