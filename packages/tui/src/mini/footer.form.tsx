...
export function RunFormBody(props: {
  ...
  const unsupported = createMemo(() => formUnsupported(props.request, props.locale))
  ...
  const action = createMemo(() => {
    if (confirm()) return t("miniCli.form.submit")
    if (textual() || state().editing) return t("miniCli.form.save")
    const field = externalField()
    if (!field) return t("miniCli.form.choose")
    if (state().answers[field.key] === true)
      return t(formSingle(props.request) ? "miniCli.form.submit" : "miniCli.form.next")
    return t(
      state().externalReady[field.key]
        ? size().width < 24
          ? "miniCli.done"
          : "miniCli.form.acknowledge"
        : "miniCli.form.open",
    )
  })
  ...
  const beginReply = async (input: FormReply) => {
    ...
    try {
      ...
      setState((previous) =>
        previous.formID === formID ? formSetError(previous, formErrorMessage(error, props.locale)) : previous,
      )
    }
  }

  const submit = (next = state()) => {
    const invalid = formValidate(props.request, next, props.locale)
    if (invalid) {
  ...
  const commitInput = () => {
    const next = formCommitInput(state(), props.request, area?.plainText ?? formInput(state(), current()), props.locale)
    setState(next)
  ...
  const moveField = (direction: -1 | 1) => {
    ...
    if (field?.type === "external") {
      if (state().answers[field.key] !== true) {
        setState((previous) => formSetError(previous, t("miniCli.form.acknowledgeField", { field: formLabel(field) })))
        return
      }
    } else if (field) {
  ...
  const external = async () => {
    ...
    try {
      ...
      setState((previous) => formSetExternalReady(previous, field.key))
      setState((previous) => formSetError(previous, t("miniCli.form.openFailed")))
    }
  }
    ...
    >
              ...
              <box width="100%" flexDirection="column" flexShrink={0} gap={compact() ? 0 : 1}>
                ...
                >
                  {state().answers[field().key] === true
                    ? t("miniCli.form.acknowledged")
                    : state().externalReady[field().key]
                      ? t("miniCli.form.enterAcknowledge")
                      : t("miniCli.form.enterOpen")}
                </text>
              </box>
            )}
          </Show>
          <Show when={!unsupported() && answerField() && !confirm()}>
            <box width="100%" flexDirection="column" flexShrink={0} gap={compact() ? 0 : 1}>
              <text width="100%" fg={props.theme.text} wrapMode="word" flexShrink={0}>
                ...
                {answerField()!.required ? t("miniCli.form.requiredSuffix") : ""}
                {multiple() ? t("miniCli.form.multipleSuffix") : ""}
              </text>
                ...
                {(field) => (
                  <text width="100%" fg={props.theme.muted} wrapMode="word" flexShrink={0}>
                    ...
                    {field.type === "external"
                      ? state().answers[field.key] === true
                        ? t("miniCli.form.acknowledged")
                        : t("miniCli.form.required")
                      : formDisplay(field, state().answers[field.key], props.locale) || t("miniCli.form.unanswered")}
                  </text>
                )}
              </For>
            </box>
      ...
      <Show when={!unsupported() && answerField() && editing()}>
        <textarea
          ...
          placeholder={formPlaceholder(answerField(), props.locale)}
      ...
      <Show
        ...
        fallback={
          <box flexDirection="row" justifyContent="space-between" gap={1} flexShrink={0}>
            ...
            >
              {state().submitting
                ? t("miniCli.form.submitting")
                : unsupported()
                  ? t("miniCli.form.dismiss")
                  : confirm()
                    ? t("miniCli.form.actionHint", { action: t("miniCli.form.submit") })
                    : editing()
                      ? t("miniCli.form.actionHint", { action: t("miniCli.form.save") })
                      : externalField()
                        ? t("miniCli.form.actionHint", { action: action() })
                        : t("miniCli.form.choiceHint", { arrows: props.mono ? "up/down" : "↑↓" })}
            </text>
        ...
        <box flexDirection="row" flexWrap="wrap" columnGap={1} flexShrink={0}>
          <Show when={!unsupported()}>
            <text
              ...
              flexShrink={0}
            >
              {state().submitting ? t("miniCli.form.submitting") : `enter ${action()}`}
            </text>
          </Show>
          ...
          <Show when={!state().submitting && size().height >= 10}>
            <text fg={props.theme.muted} wrapMode="word" maxWidth="100%" flexShrink={0}>
              {size().width >= 56 && rows().length > 0 && !state().editing ? t("miniCli.form.navigation") : ""}pgup/pgdn
              {` ${t("miniCli.scroll")}`}
            </text>
          </Show>
        </box>
