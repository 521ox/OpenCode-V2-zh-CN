...
export default function Notifications(t: Translator<Key>) {
  return Plugin.define({
    ...
    setup(context) {
      ...
      const ended = (sessionID: string) => {
        ...
        notify(context, sessionID, t("feature.notifications.done"), session?.parentID ? "subagent_done" : "done")
      }
      ...
      const dispose = [
        context.data.on("form.created", (event) => {
          ...
          notify(
            ...
            t("feature.notifications.response"),
            "question",
        ...
        context.data.on("permission.asked", (event) => {
          ...
          notify(context, event.data.sessionID, t("feature.notifications.permission"), "permission")
        }),
