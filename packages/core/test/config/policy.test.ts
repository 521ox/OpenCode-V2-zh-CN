...
describe("ConfigPolicyPlugin.Plugin", () => {
  ...
  it.effect("organization permission allow lifts authored deny without granting access", () =>
    Effect.gen(function* () {
      const managed = yield* ManagedPolicy.Service
      yield* managed.set({ statements: [permission("allow", "websearch:*")] })
      yield* addPlugin([document(permission("deny", "websearch:*"))])
      for (const effect of ["allow", "ask", "deny"] as const) {
        expect(yield* evaluate("websearch", ["private query"], effect)).toEqual({ effect, message: undefined })
      }
    }),
  )
})
